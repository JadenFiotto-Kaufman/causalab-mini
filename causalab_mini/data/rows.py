...
def column(rows: list[Row], name: str) -> list[str | int | None]:
    ...
    token id, for a metric that takes its column as ids. A row whose value
    is null or empty is an **excluded measurement** — None here — which is
    not a zero: the metric is not computed for it, and its row in the table
    says so. A column no row has at all is a misspelling, and is refused."""
    ...
    for index, row in enumerate(rows):
        ...
        values.append(None if value is None or (isinstance(value, str) and not value.strip()) else value)
    return values
