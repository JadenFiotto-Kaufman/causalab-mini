...
def _unnamed(saves: Any) -> Any:
    """A listed reference, or one mapped to null, as one whose file is still
    to be named — `Spec` names it once it knows what the reference is."""
    if isinstance(saves, list):
        return dict.fromkeys(saves, "")
    return {ref: "" if file is None else file for ref, file in saves.items()} if isinstance(saves, dict) else saves
...
class Steps(Node):
    ...
    saves: Annotated[
        dict[str, str], BeforeValidator(_unnamed, json_schema_input_type=dict[str, str | None] | list[str])
...
class Spec(Node):
    ...
    def _cross_check(self) -> "Spec":
        ...
        for ref, file in self.steps.saves.items():
            ...
            # an unnamed one is written as itself, with the extension its kind implies
            file = file or ref + (".json" if table else ".safetensors")
            _refuse(
        ...
        # frozen, but still being built: every save now has its file
        object.__setattr__(self.steps, "saves", {ref: file for file, ref in files.items()})
        return self
